...
$(document).ready(function() {
	var title = "<@spring.message "report.download.count"/>";
	var name = "<@spring.message "report.download.size"/>";
	...
	searchChart1('/downloadByCondition.do',title,name,countstr,number,'starttime','endtime','reportUnit','timeType','chart_type','#lineChart_download');
	//transmitValue('','');//参数传递
});
			...
			<div class="main-hd" style="padding-top:10px;padding-left:100px;">
			                           ...
			                           <span style="padding-left:30px;"><!-- id="searchDownloadCount" -->
			                          	    <input  type="button" class="btn btcms"  value=<@spring.message "Search"/> onclick="searchChart1('/downloadByCondition.do', '<@spring.message "report.download.count"/>','<@spring.message "report.download.size"/>','<@spring.message "report.countstr"/>','<@spring.message "number"/>','starttime','endtime','timeType','reportUnit','chart_type','#lineChart_download');"/>
									   </span>
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<div class="portlet box blue">
							<div class="portlet-body" id="lineChart_download"></div>
