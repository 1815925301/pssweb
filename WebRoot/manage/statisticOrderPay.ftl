...
$(document).ready(function() {
	var title = "<@spring.message "statistic.orderPay.title"/>";
	...
	searchChart1('/payOrderByCondition.do',title,name,countstr,number,'starttime','endtime','reportUnit','timeType','chart_type','#lineChart_pay');
	//transmitValue('','');//参数传递
});
			...
			<div class="main-hd" style="padding-top:10px;padding-left:100px;">
			                            ...
			                            <span style="padding-left:50px;"><@spring.message "report.unit"/>：
				                            <select style="width:120px" id="reportUnit">
						                            <option><@spring.message "order.paytheline"/></option>
			                           ...
			                           <span style="padding-left:30px;"><!-- id="searchDownloadCount" -->
			                          	    <input  type="button" class="btn btcms"  value=<@spring.message "Search"/> onclick="searchChart1('/payOrderByCondition.do', '<@spring.message "statistic.orderPay.title"/>','<@spring.message "report.order.ordernum"/>','<@spring.message "report.countstr"/>','<@spring.message "number"/>','starttime','endtime','reportUnit','timeType','chart_type','#lineChart_pay');"/>
									   </span>
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<div class="portlet box blue">
							<div class="portlet-body" id="lineChart_pay"></div>
